...
export default function ModelActivityIndicator() {
  const { modelAStatus, modelBStatus } = useReactionTime()
  const modelAActivity = useModelActivity(modelAStatus === 'emitting')
  const modelBActivity = useModelActivity(modelBStatus === 'reacting')
